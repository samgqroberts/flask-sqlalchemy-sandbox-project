...
def test_auth_roundtrip(client):
    ...
    res = client.post(client.url_for("auth.users"), json={'username': 'user1', 'password': 'pw'})
    assert res.status_code == 200
    assert res.data == b"Ok"

    res = client.post(client.url_for("auth.login"), json={'username': 'user1', 'password': 'pw'})
    assert res.status_code == 200
    ...
    res = client.get(client.url_for("auth.self"), headers=[('Authorization', f'Bearer {access}')])
    assert res.status_code == 200
...
def test_add_user(app, client):
    ...
    pw = 'pw1'
    res = client.post(client.url_for("auth.users"), json={'username': un, 'password': pw})
    assert res.status_code == 200
...
def test_login(app, client):
    ...
    res = client.post(client.url_for("auth.login"), json={'username': un, 'password': 'nada'})
    assert res.status_code == 401
    assert res.data == b"Bad username or password."

    res = client.post(client.url_for("auth.login"), json={'username': un, 'password': pw})
    assert res.status_code == 200
    res_json = res.get_json()
...
def test_refresh(app, client):
    ...
    res = client.post(client.url_for("auth.refresh"), headers=[('Authorization', f'Bearer nada')])
    assert res.status_code == 422
    assert res.get_json() == {'msg': 'Not enough segments'}

    res = client.post(client.url_for("auth.refresh"), headers=[('Authorization', f'Bearer {refresh_token}')])
    assert res.status_code == 200
...
def test_self(app, client):
    ...
    res = client.get(client.url_for("auth.self"), headers=[('Authorization', f'Bearer nada')])
    assert res.status_code == 422
    assert res.get_json() == {'msg': 'Not enough segments'}
