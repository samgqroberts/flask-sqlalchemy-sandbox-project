...
def test_get_thing(app, client):
    res = client.get(client.url_for('myblueprint.thing', id=1))
...
def test_update_thing(app, client):
    res = client.put(client.url_for('myblueprint.thing', id=1))
    assert res.status_code == 404
    ...
    data = {'name': 'new name'}
    res = client.put(client.url_for('myblueprint.thing', id=1), json=data)
    assert res.status_code == 200
    assert res.data == b'Ok'
    with app.app_context():
...
def test_delete_thing(app, client):
    res = client.delete(client.url_for('myblueprint.thing', id=1))
    assert res.status_code == 404
